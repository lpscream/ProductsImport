...
public class ConnectionEditForm : Form
{
    ...
    private readonly CheckBox _chkWindowsAuth = new() { Left = 140, Top = 105, Width = 260, Text = Strings.T("ConnEdit_ChkWindowsAuth") };
    ...
    private readonly Button _btnTest = new() { Left = 140, Top = 200, Width = 120, Text = Strings.T("ConnEdit_BtnTest") };
    private readonly Label _lblTestResult = new() { Left = 270, Top = 205, Width = 260, AutoSize = false };
    private readonly Button _btnOk = new() { Left = 224, Top = 240, Width = 90, Text = Strings.T("ConnEdit_BtnSave"), DialogResult = DialogResult.OK };
    private readonly Button _btnCancel = new() { Left = 320, Top = 240, Width = 80, Text = Strings.T("Common_Cancel"), DialogResult = DialogResult.Cancel };
    ...
    public ConnectionEditForm(ConnectionProfile? existing)
    {
        ...
        Text = existing == null ? Strings.T("ConnEdit_TitleNew") : Strings.T("ConnEdit_TitleEdit");
        ...
        Controls.Add(new Label { Left = 15, Top = 18, Width = 120, Text = Strings.T("ConnEdit_LblName") });
        Controls.Add(new Label { Left = 15, Top = 48, Width = 120, Text = Strings.T("ConnEdit_LblServer") });
        Controls.Add(new Label { Left = 15, Top = 78, Width = 120, Text = Strings.T("ConnEdit_LblDatabase") });
        Controls.Add(new Label { Left = 15, Top = 138, Width = 120, Text = Strings.T("ConnEdit_LblLogin") });
        Controls.Add(new Label { Left = 15, Top = 168, Width = 120, Text = Strings.T("ConnEdit_LblPassword") });
        ...
        _btnOk.Click += (_, e) =>
        {
            ...
            if (string.IsNullOrWhiteSpace(Profile.Name) || string.IsNullOrWhiteSpace(Profile.Server) || string.IsNullOrWhiteSpace(Profile.Database))
            {
                ...
                MessageBox.Show(this, Strings.T("ConnEdit_Msg_FillRequired"), Strings.T("Common_Warning"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        };
    ...
    private void TestConnection()
    {
        ...
        if (string.IsNullOrWhiteSpace(Profile.Server) || string.IsNullOrWhiteSpace(Profile.Database))
        {
            ...
            _lblTestResult.Text = Strings.T("ConnEdit_Msg_SpecifyServerAndDb");
            return;
        }
        ...
        try
        {
            ...
            _lblTestResult.Text = Strings.T("ConnEdit_Msg_ConnectionOk");
        }
        catch (Exception ex)
        {
            _lblTestResult.ForeColor = Color.DarkRed;
            _lblTestResult.Text = Strings.T("ConnEdit_Msg_ConnectionError", ex.Message);
        }
        finally
        {
