...
public class ManualBarcodeForm : Form
{
    ...
    private readonly Button _btnOk = new() { Left = 115, Top = 95, Width = 80, Text = "ОК", DialogResult = DialogResult.OK };
    private readonly Button _btnCancel = new() { Left = 200, Top = 95, Width = 80, Text = "Отмена", DialogResult = DialogResult.Cancel };

    public string Barcode => _txtBarcode.Text.Trim();

    public ManualBarcodeForm(string productName, string? currentValue)
    {
        Text = "Ввод штрих-кода вручную";
        ...
        Controls.Add(new Label { Left = 15, Top = 12, Width = 260, Text = $"Товар: {productName}" });
        ...
        _btnOk.Click += (_, e) =>
        {
            ...
            {
                _lblError.Text = "Штрих-код должен содержать 8, 12 или 13 цифр.";
